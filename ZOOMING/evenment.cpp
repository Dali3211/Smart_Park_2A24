...
QSqlQueryModel* evenment::triI()
{
    QSqlQueryModel * model= new QSqlQueryModel();
           model->setQuery("SELECT * FROM evenment ORDER BY IDEV");
           return model;
}
...
QSqlQueryModel* evenment::rechercherA()
{
        QSqlQueryModel* model=new QSqlQueryModel();
        model->setQuery("select DATEEV from evenment");
        return model;
